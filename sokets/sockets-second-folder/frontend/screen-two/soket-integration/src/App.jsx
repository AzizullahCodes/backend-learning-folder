   ...
   <div className="chat-container">
     ...
     <div className="chat-body">
       <ul className="message-list">
        {
          allMessages?.map((item,index)=>{
            const isMine = item.from === 'user_2';
            return (
              <li key={index} className={`message-bubble-wrapper ${isMine ? 'sent' : 'received'}`}>
                <div className="message-bubble">
                  <span className="message-sender">{item.from}</span>
                  <p className="message-text">{item.message}</p>
                  <div className="message-actions">
                    <button className="btn-icon delete" onClick={()=>deleteHandler(index)}>🗑</button>
                    {item.from == 'user_2' && (
                      <button className="btn-icon edit" onClick={()=>editHandler(item,index)}>✎</button>
                    )}
                  </div>
                </div>
              </li>
     ...
     <div className="chat-input-area">
       ...
       />
      {isEdit && <button className="btn-send btn-update" onClick={updateHandler}>Update</button>}
      {!isEdit && <button className="btn-send" onClick={addMessage}>Send</button>}
     </div>
   </div>
